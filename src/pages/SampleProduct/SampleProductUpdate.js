...
const SampleProductUpdate = (props) => {
  ...
  const [oldMultipleImages, setOldMultipleImages] = useState(
    PrmaryImfomation.product_multiple_image != '[]' ? PrmaryImfomation.product_multiple_image?.split(",") : []
  );
  ...
  const SubmitData = () => {
    ...
    formData.append("kept_multiple_images", oldMultipleImages.length > 0 ? oldMultipleImages.join(",") : null);
    ...
    if (checkPrice_sales) {
      if (PrmaryImfomation.product_english_name == "") {
        ...
        http
          ...
          .then(function (response) {
            https.post("/products/update-images", formData)
              .then((res) => {
...
export default SampleProductUpdate;
