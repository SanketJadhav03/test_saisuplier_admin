...
const WhatsAppButton = ({ billDetails, color }) => {
  const { http } = AuthUser();
  ...
  useEffect(() => {
    const sendWhatsAppMessage = async () => {
      try {
        ...
        const message = `${
          companyDetailsResponse.data[0].business_name
        }मध्ये खरेदी केल्याबद्दल धन्यवाद.

आपले बिल क्र. ${billDetails.master_invoice_no}
बिल रक्कम रु. ${billDetails.master_total_bill_amt}
बचत रक्कम रु. ${
          billDetails.master_total_bill_mrp - billDetails.master_total_bill_amt
        }
...
शुभ दिवस पुन्हा भेट द्या!`;
  ...
  if (PhoneNumber) {
    return (
      <a
        href={`https://wa.me/91${PhoneNumber}/?text=${EncodedMessage}`}
