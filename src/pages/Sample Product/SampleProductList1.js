...
import SampleProductAdd from "./SampleProductAdd1";
import SampleProductUpdate from "./SampleProductUpdate2";
import { useEffect } from "react";
import AuthUser from "../../helpers/Authuser";
import SampleProductView from "./SampleProductView1";
import D_img from "../D_img";
import { IMG_API_URL } from "../../helpers/url_helper";

const SampleProductList1 = () => {
              ...
              <CardHeader className="card-header border-0">
                <Row className="align-items-center gy-3">
                  <div className="col-sm">
                    <h5 className="card-title mb-0">Sample Product List</h5>
                  </div>
                  ...
                  <div className="col-sm-auto">
                    <div className="d-flex gap-1 flex-wrap">
                      ...
                      />
                      {permission.find(permission => permission.permission_category === "PRODUCTS" && permission.permission_path === "2")
                        &&
                        <button
                          type="button"
                      ...
                      <tbody>
                        {ProductData.map((item, index) => (
                          <tr key={index}>
                            ...
                            <td>
                              {item.sample_product_image ? (
                                <img
                                  src={`${IMG_API_URL}/products/${item.sample_product_image}`}
                                  alt="old-product-img"
                                  className="h-auto"
                                  width={"100px"}
                                />
                            ...
                            <td>{item.sample_product_english_name}</td>
                            <td>{item.sample_product_mrp}</td>
                            <td>{item.sample_product_sale_price}</td>
                            <td>
                              <ul className="list-inline hstack gap-2 mb-0">
                                {checkPermission("Products Show") ? (
                                ...
                                {permission.find(permission => permission.permission_category === "PRODUCTS" && permission.permission_path === "3")
                                  && <li className="list-inline-item edit">
                                    <button
                                      className="text-primary d-inline-block edit-item-btn border-0 bg-transparent"
                                ...
                                {permission.find(permission => permission.permission_category === "PRODUCTS" && permission.permission_path === "4")
                                  &&
                                  <li className="list-inline-item">
                                    <button
                                      onClick={() =>
                                        onClickDelete(item.sample_product_id)
                                      }
...
export default SampleProductList1;
