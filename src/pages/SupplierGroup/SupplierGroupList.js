...
import SupplierGroupAdd from "./SupplierGroupAdd";
import SupplierGroupUpdate from "./SupplierGroupUpdate";
...
const SupplierGroupList = () => {
  ...
  const handleDeleteOrder = (data) => {
    if (data._reactName == "onClick") {
      http
        .delete(`/supplier_group/delete/${ID}`)
  ...
  useEffect(() => {
    document.title = "Supplier Group  | Saisupplier Admin";

    http
      .get(`/supplier_group/list?page=${Pages}&limit=30`)
      .then(function (response) {
        setSupplierGroup([...SupplierGroup, ...response.data]);
        SetPages(Pages + 1);
  ...
  const EditUpdate = (index) => {
    let FindArray = SupplierGroup.filter((_, i) => i == index);
            ...
            <Card>
              <CardHeader className="card-header border-0">
                <Row className="align-items-center gy-3">
                  <div className="col-sm">
                    <h5 className="card-title mb-0">SupplierGroup </h5>
                  </div>
              ...
              <CardBody className="pt-0">
                <div>
                  ...
                  ></Nav>
                  <InfiniteScroll
                    dataLength={SupplierGroup.length}
                    next={fetchData}
                    hasMore={NoMore}
                  >
                    <table
                      ...
                      <tbody>
                        {SupplierGroup.map((item, index) => (
                          <tr key={index}>
                            ...
                            <td>{item.supplier_group_name}</td>
                              ...
                              <ul className="list-inline hstack gap-2 mb-0">
                                ...
                                <li className="list-inline-item">
                                  {item.supplier_group_id != 1 ? (
                                    <button
                                      onClick={() =>
                                        onClickDelete(item.supplier_group_id)
                                      }
