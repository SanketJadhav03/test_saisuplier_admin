...
const CustomerGroupList = () => {
  ...
  const handleDeleteOrder = (data) => {
    if (data._reactName == "onClick") {
      http
        .delete(`/customer_group/delete/${ID}`)
  ...
  useEffect(() => {
    http
      .get(`/customer_group/list?page=${Pages}&limit=30`)
      .then(function (response) {
        setCustomerGroup([...customerGroup, ...response.data]);
        SetPages(Pages + 1);
  ...
  const EditUpdate = (index) => {
    let FindArray = customerGroup.filter((_, i) => i == index);
  ...
  useEffect(() => {
    document.title = "Customer Group | Saisupplier Admin";
            ...
            <Card>
              <CardHeader className="card-header border-0">
                <Row className="align-items-center gy-3">
                  <div className="col-sm">
                    <h5 className="card-title mb-0">CustomerGroup </h5>
                  </div>
              ...
              <CardBody className="pt-0">
                <div>
                  ...
                  ></Nav>
                  <InfiniteScroll
                    dataLength={customerGroup.length}
                    next={fetchData}
                    hasMore={NoMore}
                  >
                    <table
                              ...
                              <ul className="list-inline hstack gap-2 mb-0">
                                ...
                                <li className="list-inline-item">
                                  {item.customer_group_id != 1 ? (
                                    <button
                                      onClick={() =>
                                        onClickDelete(item.customer_group_id)
                                      }
