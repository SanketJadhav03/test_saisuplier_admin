...
const PurchaseUpdate = () => {
  ...
  const StoreDataPrice = (data) => {
    ...
    if (existingIndex !== -1) {
      ...
      const Qty = parseInt(dataUP.qty) + 1;
      ...
      updatedData[existingIndex] = {
        ...
        qty: Qty,
        dis_pre: dis_pre,
        dis_value: dis_values.toFixed(2),
      ...
      const Qty = 1; // Initial quantity as integer
  ...
  const ChangInput = (e, index, field, check) => {
    ...
    if (field === "purchase_note") {
      updatedProduct.purchase_note = e.target.value;
    } else if (check === 1) {
  ...
  useEffect(() => {
    ...
    for (const item of Data_View) {
      ...
      Total_purchses += parseFloat(item.price_purchase);
  ...
  const updateProductNotes = (index, value) => {
    Data_View[index].purchase_note = value;
  };
                              ...
                              <div className="form-icon right rounded d-flex bg-primary align-items-center">
                                <div className="w-100">
                                  <Select
                                    ...
                                    value={{
                                      label:
                                        BasicInformtion.customer.find(
                                          (c) =>
                                            c.user_id ==
                                            MasterArray.purchase_customer_id,
                                        )?.user_type == 1
                                          ? BasicInformtion.customer.find(
                                              (c) =>
                                                c.user_id ==
                                      ...
                                      value: BasicInformtion.customer.find(
                                        (c) =>
                                          c.user_id ==
                                          MasterArray.purchase_customer_id,
                                      )?.user_id,
                                  ...
                                  <tr key={index}>
                                    ...
                                    <td>
                                      <input
                                        ...
                                        onChange={(e) => {
                                          ChangInput(e, index, "purchase_note");
                                        }}
                                        className="form-control"
                                        placeholder={
                                          "Add " +
                                      ...
                                      {item.attachment ? (
                                        <>
                                          <button
                                            ...
                                            onClick={() => {
                                              if (item.preview) {
                                                setPreview(item.preview); // new file
                                              } else {
                                                setPreview(
